...
export async function analyzeRequest(
  ...
  return await sendAnalyze(text, base64Image, accessToken, history, session_id);
}
...
export async function performChatRequest(params: {
  ...
  const { ok, status, payload } = await analyzeRequest(params.text, params.base64Image, params.accessToken, params.history, params.session_id);
