...
type PenalizeResult = { user_id: string; ok: boolean; error?: string };
...
export async function GET(req: Request) {
  try {
    ...
    try {
      ...
      if (error) {
        return jsonError(503, 'database_unavailable', `Failed to list users: ${error.message}`);
      }
    ...
    const { data: logsToday, error: logsError } = await supabase.from('daily_logs').select('user_id').eq('date', today);
    if (logsError) return jsonError(503, 'database_unavailable', `Failed to read daily logs: ${logsError.message}`);
    ...
    for (const uid of inactive) {
      try {
        ...
        const { error: resetError } = await supabase.from('user_habits').update({ current_streak: 0 }).eq('user_id', uid);
        if (resetError) {
