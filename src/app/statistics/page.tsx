...
const CalendarHeart = (props: SVGProps<SVGSVGElement>) => (
...
export default async function StatisticsPage({ searchParams }: StatisticsPageProps) {
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
    return (
      <div className="flex-1 overflow-y-auto pb-4 md:pb-8 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.98),_rgba(233,238,244,0.95)_38%,_rgba(212,220,230,0.96)_100%)] px-4 py-6 text-slate-900 sm:px-6 lg:px-8 scrollbar-hide">
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
          <header className="rounded-[2rem] border border-white/80 bg-white/75 px-5 py-5 shadow-[0_22px_70px_rgba(15,23,42,0.12)] backdrop-blur-2xl sm:px-6">
            <p className="text-[10px] uppercase tracking-[0.38em] text-slate-500">Estadísticas</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Modo local sin Supabase</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              Esta vista necesita credenciales reales para leer tendencias, archivo diario y evolución corporal.
            </p>
          </header>
        </div>
      </div>
  ...
  const [{ data }, { data: moodData }, { data: habitsData }, { data: bodyMetricsData }, { data: workoutsData }] = await Promise.all([
    ...
    supabaseReal
      ...
      .order('date', { ascending: false })
      .limit(180),
    supabaseReal.from('user_habits').select('*').eq('user_id', user.id),
    supabaseReal.from('body_metrics').select('*').eq('user_id', user.id).order('date', { ascending: true }).limit(180),
    supabaseReal.from('workouts').select('*').eq('user_id', user.id).gte('date', startDate).lte('date', endDate).order('date', { ascending: false }).limit(180),
  ]);
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <header className="rounded-[2rem] border border-white/80 bg-white/75 px-5 py-5 shadow-[0_22px_70px_rgba(15,23,42,0.12)] backdrop-blur-2xl sm:px-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.38em] text-slate-500">Estadísticas</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Tendencias, archivo y cuerpo</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                Una lectura clara de tus patrones diarios, el archivo desplegable y la evolución corporal en el mismo lugar.
              </p>
            </div>
            <Link
              href="/"
        ...
        {hasLogs ? (
          ...
          <section className="flex min-h-[50vh] items-center justify-center px-2">
            <div className="max-w-xl rounded-[2rem] border border-white/80 bg-white/80 p-8 text-center shadow-[0_20px_70px_rgba(15,23,42,0.12)] backdrop-blur-2xl">
              ...
              <h2 className="mt-5 text-2xl font-semibold text-slate-950">Tu archivo se activará con el uso</h2>
              <p className="mt-3 text-sm leading-6 text-slate-600">
                Cierra tu primer día o registra tu primera medición corporal para encender esta vista.
              </p>
            </div>
          </section>
        )}
