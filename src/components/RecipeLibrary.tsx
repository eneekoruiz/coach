...
export default function RecipeLibrary() {
  ...
  const handleSaveRecipe = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(`Error al guardar la receta: ${error instanceof Error ? error.message : 'Error unexpected'}`);
    }
  };
  ...
  const handleDeleteRecipe = async (recipe: Recipe) => {
    ...
    try {
      ...
      toast.error(`Error al eliminar la receta: ${error instanceof Error ? error.message : 'Error unexpected'}`);
    }
  };
  ...
  return (
    <div className="h-[72dvh] min-h-0 overflow-y-auto pr-1 md:overflow-hidden md:pr-0">
      {!isDetailActive ? (
        /* Library View */
        <div className="h-full flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm max-w-2xl mx-auto animate-fade-in">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <Soup className="h-5 w-5 text-emerald-600" />
              <div>
          ...
          {/* Prominent Action Button (Gigante y Limpio) */}
          <button
            type="button"
            onClick={startNewRecipe}
            className="mt-4 flex w-full min-h-[52px] items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 px-5 text-sm font-black text-white hover:from-emerald-700 hover:to-teal-700 active:scale-95 transition-all shadow-md shrink-0 animate-pulse-subtle"
          >
          ...
          <div className="relative mt-3 shrink-0">
            ...
            <input
              ...
              className="h-11 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-xs font-bold text-slate-700 outline-none transition focus:border-slate-400 focus:bg-white"
            />
          </div>
            ...
            {availableTags.map((tag) => {
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap rounded-full px-3.5 py-2 text-[10px] font-black transition-all duration-200 ease-in-out ${
                    active
          ...
          <div className="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1 scrollbar-hide">
            {loading ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center">
                ...
                <p className="mt-3 text-xs font-bold text-slate-500">Sin recetas guardadas</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {filteredRecipes.map(({ recipe, tags }) => (
                  <button
                    ...
                    className="group w-full rounded-2xl border border-slate-200 bg-slate-50/50 p-4 text-left transition hover:border-slate-350 hover:bg-white hover:shadow-sm"
                  >
                    <div className="flex flex-col h-full justify-between gap-3">
                      <div>
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate text-xs font-black text-slate-950">{recipe.name}</p>
                      ...
                      {tags.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {tags.slice(0, 2).map((tag) => (
                            <span
                              key={tag}
                              className="rounded-full bg-white px-2 py-0.5 text-[8px] font-black text-slate-500 ring-1 ring-slate-200"
                            >
                              {tag}
                            </span>
                          ))}
        ...
        /* Form Creation Form / Canvas */
        <div className="h-full flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm max-w-4xl mx-auto overflow-y-auto custom-scrollbar">
          <button
            type="button"
